...
%>
<table id="catetory-down" title="栏目信息列表" class="easyui-datagrid"
		url="<%=basePath%>admin/catetory/downList.action" fit="true" pageSize="30"
		toolbar="#catetory-down-toolbar" pagination="true" collapsible="true"
...
<div id="catetory-down-toolbar" style="height:50px">
...
<div id="catetory-down-dlg" class="easyui-dialog" style="width: 400px; height: 180px; padding: 10px 20px"
    closed="true" buttons="#catetory-down-dlg-buttons" shadow="false">
    <form id="catetory-down-fm" method="post">
        <div class="fitem">
            <label>栏目名称:</label> 
...
<div id="catetory-down-dlg-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveCatetoryDown()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#catetory-down-dlg').dialog('close')">取消</a>
</div>
<script type="text/javascript">
//查询参数
function searchCatetoryDown(){
	$('#catetory-down').datagrid('load',{
...
function newCatetoryDown(){
    $('#catetory-down-dlg').dialog('open').dialog('setTitle','新增栏目');
    $('#catetory-down-fm').form('clear');
    url = '<%=basePath%>admin/catetory/add.action';
}
//编辑
function editCatetoryDown(){
	var row = $('#catetory-down').datagrid('getSelected');
	if (row){
		$('#catetory-down-dlg').dialog('open').dialog('setTitle','编辑栏目');
		$('#catetory-down-fm').form('load',row);
	...
	$('#catetory-down-fm').form('submit',{
		...
		success: function(date){
			...
			if (date.result == 'error'){
				...
				$('#catetory-down-dlg').dialog('close');	
				showInfo("操作成功!");
				$('#catetory-down').datagrid('reload');	
			}
		}
	});
}
...
function batchDesCatetoryDown() {
    var selRow = $("#catetory-down").datagrid('getSelections');
    if (selRow.length == 0) {
        showInfo("请至少选择一行数据!");
        return false;
    }
