...
%>
<table id="user-warn" title="用户警告列表" class="easyui-datagrid"
		url="<%=basePath%>admin/user/warnList.action" fit="true" pageSize="30"
		toolbar="#user-warn-toolbar" pagination="true" collapsible="true"
...
<div id="user-warn-toolbar" style="height:80px">
	...
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="searchUserWarn()">搜索</a> <br>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editUserWarn()">编辑</a>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="destroyUserWarn()">删除</a>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-back" plain="true" onclick="recoverWarnUserWarn()">撤销警告</a>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="lockUserWarn()">拉黑</a>
</div>

<div id="user-warn-dlg" class="easyui-dialog" style="width: 400px; height: 300px; padding: 10px 20px"
    closed="true" buttons="#user-warn-dlg-buttons" shadow="false">
    <form id="user-warn-fm" method="post">
        <div class="fitem">
            <label>学号:</label> 
...
<div id="user-warn-dlg-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveUserWarn()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#user-warn-dlg').dialog('close')">取消</a>
</div>
<script type="text/javascript">
function searchUserWarn(){
	$('#user-warn').datagrid('load',{
...
function editUserWarn(){
	var row = $('#user-warn').datagrid('getSelected');
	if (row){
		$('#user-warn-dlg').dialog('open').dialog('setTitle','编辑用户');
		$('#user-warn-fm').form('load',row);
		url = '<%=basePath%>admin/user/update.action?userId='+row.userId;
	}else{
		showInfo("请选择一行数据!");
	}
}
function saveUserWarn(){
	$('#user-warn-fm').form('submit',{
		url: url,
		onSubmit: function(){
		...
		success: function(date){
			...
			if (date.result == 'error'){
				...
				$('#user-warn-dlg').dialog('close');	
				showInfo("操作成功!");
				$('#user-warn').datagrid('reload');	
			}
		}
	});
}
function destroyUserWarn(){
	var row = $('#user-warn').datagrid('getSelected');
	if (row){
...
//批量删除
function batchDesUserWarn() {
    var selRow = $("#user-warn").datagrid('getSelections');
    if (selRow.length == 0) {
        showInfo("请至少选择一行数据!");
        return false;
    }
...
//撤销警告
function recoverWarnUserWarn(){
	var row = $('#user-warn').datagrid('getSelected');
	if (row){
		$.messager.confirm('撤销警告用户','确定撤销警告？',function(r){
			if (r){
				$.post(
					'<%=basePath%>admin/user/recoverWarn.action',
					{userId:row.userId},
					function(date){
						if (date.result == 'success'){
							$('#user-warn').datagrid('reload');	
...
//拉黑用户
function lockUserWarn(){
	var row = $('#user-warn').datagrid('getSelected');
	if (row){
		$.messager.confirm('拉黑用户','确定拉黑该用户？',function(r){
			if (r){
				$.post(
					'<%=basePath%>admin/user/lock.action',
					{userId:row.userId},
					function(date){
						if (date.result == 'success'){
							$('#user-warn').datagrid('reload');	
